...
### Station and element selection (config-driven)
...
target stations reference this same table.
**Adding a 6th city means adding one row to this CSV.**
...
dataset (`TMAX`, `TMIN`, `TAVG`, `PRCP`, `WSFG`). A more complete solution would use a
small seed file mapping element code to scale factor joined in. This was
deferred as out of scope for the target five station dataset (see Tradeoffs below).
...
- **`int_observations_enriched`** keeps every row, including flagged ones, and adds an explicit
  `failed_quality_check` boolean column. This preserves full traceability, nothing discarded at this layer,
  and the specific QFLAG code (e.g. `I` = failed internal consistency
...
`dbt_project.yml`. The end date was chosen as the latest date with
...
- Is instructed to omit secondary elements (like wind direction in degrees) rather than force
  them into overly technical phrasing.
...
alongside the narrative in DuckDB. See Tradeoffs below for a known limitation of this guardrail.
...
- **Automated dbt tests** on `fct_daily_weather`: `not_null` on key columns, `accepted_values`
  on `city` (flagged in the test itself as scoped to the initial target of 5 cities), and
...
- **Reproducibility test**: the full pipeline (ingestion → dbt → narratives) was run from a
  completely deleted database to confirm it rebuilds cleanly end-to-end. 

---

## Tradeoffs & What I'd Improve With More Time

- **Unit scaling coverage**: currently include the 8 elements known to appear in this dataset.
...
- **Validation guardrail precision**: the guardrail occasionally flags narratives as invalid due
  ...
  iteration would tune the validation prompt's tolerance, or add a secondary review step before
  treating `is_valid = false` as a hard signal.
